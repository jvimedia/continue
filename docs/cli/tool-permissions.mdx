...
Every tool has one of five permission levels:

| Permission | Behavior |
|------------|----------|
| `allow` | Runs automatically, no prompt |
| `ask` | Prompts for approval before running (TUI only) |
| `review` | Sends the tool call to the current AI, without previous conversation context; runs if the AI allows it and denies if the AI sees risk |
| `reviewAsk` | Sends the tool call to the current AI first; denies if the AI sees risk, otherwise prompts the user before running |
...
Use `--allow`, `--ask`, `--review`, `--review-ask`, and `--exclude` to override defaults at launch:
...
# Let the current AI review terminal commands before running them
cn --review Bash

# Let the current AI block risky terminal commands, then ask before running safe ones
cn --review-ask Bash
